...
OUTPUT_FILE = Path(
    "data/tracking/initial_score_ver3_candidate_analysis.csv"
)
...
def build_masks(df):
    ...
    # 出来高増加日数
    # --------------------------------------------------------
    masks["出来高増加1日"] = df["VolumeIncreaseDays"] >= 1
    masks["出来高増加2日"] = df["VolumeIncreaseDays"] >= 2
    masks["出来高増加3日"] = df["VolumeIncreaseDays"] >= 3

    # --------------------------------------------------------
    # ブレイク系
    # --------------------------------------------------------
    masks["ブレイク"] = df["BreakoutSignal"].fillna(False).astype(bool)
    masks["ブレイク初日"] = df["BreakoutFirstDay"].fillna(False).astype(bool)
    masks["30日高値更新"] = df["New30High"].fillna(False).astype(bool)

    # --------------------------------------------------------
    # トレンド系
    # --------------------------------------------------------
    masks["MA5上"] = df["AboveMA5"].fillna(False).astype(bool)
    masks["MA25上"] = df["AboveMA25"].fillna(False).astype(bool)
    masks["MA75上"] = df["AboveMA75"].fillna(False).astype(bool)
    masks["MACD GC"] = df["MACD_GC"].fillna(False).astype(bool)
...
def calc_base_score(df, masks, scheme):
    ...
    # 「最高ランクのみ」加点
    # --------------------------------------------------------
    increase_points = scheme["increase_points"]

    increase_score = (
        masks["出来高増加1日"].astype(float) * increase_points[1]
    )

    increase_score = increase_score.where(
        ~masks["出来高増加2日"],
        increase_points[2],
    )
